...
Strategy_Name = 'DonchianChannel_copy'
...
def init_per_day(sdk):
    ...
    if removed_stock:
        ...
        stock_with_position += stock_with_position_today
    ...
    not_stop_removed = list(set(pd.Series(stock_list)[np.logical_and(not_stop, on_list)]).intersection(set(stock_with_position)))
    ...
    # 找到所有可交易股票前10天的最高价和前5天的最低价
    high = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_THIGH')[-10:], columns=stock_list)[stock_observe]
    low = pd.DataFrame(sdk.getFieldData('LZ_GPA_QUOTE_TLOW')[-5:], columns=stock_list)[stock_observe]
    up_line = high.max(axis=0)
    down_line = low.min(axis=0)
    # 记录
    sdk.sdklog(today, '==========================日期')
